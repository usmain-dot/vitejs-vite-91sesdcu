const translation = {
  "app": {
    "name": "جسر",
    "tagline": "ربط المجتمعات بالخدمات الأساسية"
  },
  "nav": {
    "signIn": "تسجيل الدخول",
    "logout": "تسجيل الخروج",
    "about": "حول",
    "admin": "المشرف"
  },
  "search": {
    "placeholder": "البحث عن الخدمات...",
    "showing": "عرض {{count}} من {{total}} خدمة"
  },
  "categories": {
    "all": "جميع الخدمات",
    "housing": "الإسكان",
    "healthcare": "الرعاية الصحية",
    "legal": "المساعدة القانونية",
    "employment": "التوظيف",
    "education": "التعليم",
    "food": "المساعدات الغذائية",
    "language": "دروس اللغة",
    "mentalHealth": "الصحة النفسية",
    "childcare": "رعاية الأطفال"
  },
  "service": {
    "open": "مفتوح",
    "closed": "مغلق",
    "callNow": "اتصل الآن",
    "getDirections": "الحصول على الاتجاهات",
    "sendMessage": "إرسال رسالة",
    "bookAppointment": "حجز موعد",
    "miAway": "ميل بعيدًا",
    "noResults": "لم يتم العثور على خدمات. حاول تعديل الفلاتر.",
    "selectPrompt": "اختر فئة أو ابحث للعثور على الخدمات",
    "browsePrompt": "تصفح الإسكان والرعاية الصحية والمساعدة القانونية والمزيد",
    "loading": "جارٍ تحميل الخدمات..."
  },
  "phases": {
    "phase2Title": "ميزات المرحلة الثانية",
    "phase2Subtitle": "ميزات محسّنة قادمة قريبًا",
    "phase3Title": "ميزات المرحلة الثالثة",
    "phase3Subtitle": "إمكانيات متقدمة للدعم الشخصي",
    "comingSoon": "قريبًا",
    "messaging": "المراسلة المباشرة",
    "messagingDesc": "دردشة فورية مع مزودي الخدمات للأسئلة والدعم",
    "scheduling": "جدولة المواعيد",
    "schedulingDesc": "احجز وأدر مواعيدك مباشرةً عبر المنصة",
    "forums": "المنتديات المجتمعية",
    "forumsDesc": "تواصل مع أعضاء المجتمع وشارك تجاربك",
    "aiMatching": "مطابقة الذكاء الاصطناعي",
    "aiMatchingDesc": "توصيات مدعومة بالذكاء الاصطناعي بناءً على احتياجاتك",
    "documents": "تخزين المستندات",
    "documentsDesc": "تخزين آمن للمستندات والسجلات المهمة",
    "caseManagement": "إدارة الحالات",
    "caseManagementDesc": "تتبع رحلتك مع الخدمات وتوثيق التقدم"
  },
  "footer": {
    "rights": "© 2025 Bridge.",
    "serving": "نخدم المجتمعات في جميع أنحاء ولاية نيويورك"
  },
  "common": {
    "close": "إغلاق",
    "loading": "جارٍ التحميل..."
  },
  "coworking": {
  "title": "مساحات العمل المشترك في نيويورك",
  "subtitle": "اعثر على مساحات عمل مرنة — من المكاتب المشتركة إلى المكاتب الخاصة — عبر أحياء مدينة نيويورك الخمسة",
  "spaceType": "نوع المساحة",
  "borough": "الحي",
  "allBoroughs": "جميع الأحياء",
  "showing": "عرض",
  "of": "من",
  "spaces": "مساحات",
  "selectPrompt": "اختر نوع مساحة للاستكشاف",
  "selectSub": "مؤسسي، مميز، مجتمعي والمزيد",
  "noResults": "لم يتم العثور على مساحات. حاول تعديل الفلاتر.",
  "viewLocations": "عرض المواقع",
  "directions": "الاتجاهات",
  "multiLocation": "مواقع متعددة في NYC متاحة",
  "tierAll": "الكل",
  "tierEnterprise": "مؤسسي",
  "tierPremium": "مميز",
  "tierCommunity": "مجتمعي",
  "tierPopular": "الأكثر شيوعاً",
  "boroughManhattan": "مانهاتن",
  "boroughBrooklyn": "بروكلين",
  "boroughQueens": "كوينز",
  "boroughBronx": "برونكس",
  "boroughStatenIsland": "ستاتن آيلاند",
  "boroughAll": "جميع الأحياء"
},
"online": {
  "title": "الموارد الإلكترونية",
  "subtitle": "مواقع وأدوات أساسية لمساعدتك على الاستقرار في نيويورك",
  "visitWebsite": "زيارة الموقع",
  "housing": "السكن والإيجار",
  "utilities": "المرافق والإنترنت",
  "community": "المجتمع والفعاليات",
  "housingDesc": "ابحث عن شقق وسكن ميسور التكلفة واعرف حقوقك كمستأجر",
  "utilitiesDesc": "قم بإعداد الكهرباء والغاز والإنترنت لمنزلك الجديد",
  "communityDesc": "تعرف على أشخاص جدد واعثر على فعاليات ووسّع شبكتك المهنية",
  "selectPrompt": "اختر فئة للاستكشاف",
  "selectSub": "موارد السكن والمرافق والمجتمع متاحة",
  "tagRentSearch": "البحث عن إيجار",
  "tagAffordable": "ميسور التكلفة",
  "tagTenantRights": "حقوق المستأجر",
  "tagSection8": "القسم 8",
  "tagElectricityGas": "الكهرباء والغاز",
  "tagGas": "الغاز",
  "tagInternet": "الإنترنت",
  "tagFreeWifi": "واي فاي مجاني",
  "tagGovernment": "حكومي",
  "tagNetworking": "التواصل المهني",
  "tagEvents": "فعاليات",
  "tagFreeEvents": "فعاليات مجانية",
  "tagFreeClasses": "دروس مجانية",
  "tagCareer": "مهنة",
  "tagJobs": "وظائف"
},
"schools": {
  "title": "المدارس في نيويورك",
  "subtitle": "ابحث عن الجامعات والكليات والمدارس في جميع الأحياء الخمسة",
  "schoolType": "نوع المدرسة",
  "borough": "الحي",
  "allBoroughs": "جميع الأحياء",
  "languageLabel": "لغة التدريس",
  "allLanguages": "جميع اللغات",
  "showing": "عرض",
  "of": "من",
  "schools": "مدارس",
  "selectPrompt": "اختر نوع مدرسة للاستكشاف",
  "selectSub": "جامعات، كليات، مدارس ثانوية والمزيد",
  "noResults": "لم يتم العثور على مدارس. حاول تعديل الفلاتر.",
  "grades": "الصفوف",
  "visitWebsite": "زيارة الموقع",
  "directions": "الاتجاهات",
  "typeAll": "جميع المدارس",
  "typeUniversity": "الجامعات",
  "typeCollege": "الكليات",
  "typeHighSchool": "المدارس الثانوية",
  "typeMiddleSchool": "المدارس المتوسطة",
  "typePrimarySchool": "المدارس الابتدائية",
  "langEnglish": "الإنجليزية",
  "langSpanish": "الإسبانية",
  "langArabic": "العربية",
  "langChinese": "الصينية",
  "langFrench": "الفرنسية",
  "boroughManhattan": "مانهاتن",
  "boroughBrooklyn": "بروكلين",
  "boroughQueens": "كوينز",
  "boroughBronx": "برونكس",
  "boroughStatenIsland": "ستاتن آيلاند",
  "boroughAll": "جميع الأحياء"
},
"about": {
  "title": "حول Bridge NYC",
  "badge": "عن Bridge NYC",
  "headline1": "نيويورك مدينة كبيرة.",
  "headline2": "إيجاد المساعدة لا ينبغي أن يكون صعباً.",
  "intro": "Bridge منصة مجانية تربط المهاجرين واللاجئين والقادمين الجدد في مدينة نيويورك بالسكن والرعاية الصحية والمساعدة القانونية والمدارس والتوظيف والمزيد — كل ذلك في مكان واحد، بلغتك.",
  "missionBadge": "مهمتنا",
  "missionTitle1": "منصة واحدة.",
  "missionTitle2": "كل الموارد التي تحتاجها.",
  "missionP1": "عندما تصل إلى مدينة جديدة، كل شيء غير مألوف — اللغة والنظام والناس. الخدمات الأساسية موزعة على عشرات المواقع وأرقام الهواتف والمكاتب، جميعها بالإنجليزية.",
  "missionP2": "Bridge يغير ذلك. نجمع مساعدة السكن والمساعدة القانونية المجانية وعيادات الرعاية الصحية وفصول اللغة الإنجليزية وأفضل المدارس ومساحات العمل المرنة في منصة متعددة اللغات مع الاتجاهات وأرقام الهواتف وحجز المواعيد.",
  "whyBadge": "لماذا Bridge",
  "whyTitle": "ما يميزنا",
  "whoBadge": "لمن هو",
  "whoTitle": "Bridge مصنوع لك إذا كنت…",
  "ctaTitle": "هل تعرف خدمة يجب إدراجها؟",
  "ctaSubtitle": "Bridge ينمو مع المجتمع. إذا كنت تعرف موردًا مفقودًا، ساعدنا في إضافته.",
  "ctaButton": "اقتراح خدمة",
  "statsSchools": "المدارس المدرجة",
  "statsServices": "الخدمات الأساسية",
  "statsCoworking": "مساحات العمل المشترك",
  "statsLanguages": "اللغات المدعومة",
  "pillars": [
    { "title": "مبني للقادمين الجدد", "desc": "مصمم خصيصاً للمهاجرين واللاجئين وطالبي اللجوء الذين يتنقلون في نيويورك لأول مرة." },
    { "title": "متعدد اللغات", "desc": "متاح بالإنجليزية والإسبانية والعربية والعبرية والكيسواحيلي — لأن اللغة لا ينبغي أن تكون عائقاً للحصول على المساعدة." },
    { "title": "مجاني وموثوق", "desc": "كل خدمة مدرجة يتم التحقق منها ومجانية. لا إعلانات، لا جدران دفع — فقط مساعدة حقيقية لأشخاص حقيقيين." },
    { "title": "المجتمع أولاً", "desc": "مبني من قِبل المجتمعات التي يخدمها ولها. إذا كانت هناك خدمة مفقودة، يمكنك مساعدتنا في إضافتها." }
  ],
  "services": [
    "🏠 مأوى طارئ وإسكان انتقالي",
    "⚖️ مساعدة قانونية مجانية للهجرة والسكن",
    "🏥 رعاية صحية متعددة اللغات وصحة نفسية",
    "🎓 أكثر من 80 مدرسة في جميع الأحياء الخمسة",
    "💼 تدريب مهني وإنجليزية وخدمات التوظيف",
    "🖥️ مساحات عمل مشترك للمقاولين"
  ],
  "whoTags": [
    "🛬 مهاجر حديث", "🔒 طالب لجوء", "🌍 لاجئ",
    "📚 يبحث عن مدارس", "🏠 يبحث عن سكن", "⚖️ يحتاج مساعدة قانونية",
    "💊 يبحث عن رعاية صحية", "💼 يبحث عن عمل", "🖥️ عامل عن بُعد",
    "🗣️ يتعلم الإنجليزية", "👨‍👩‍👧 عائلة تستقر في NYC", "🚀 رائد أعمال مهاجر"
  ]
},
"auth": {
  "loginToMessage": "سجّل الدخول لإرسال الرسائل",
  "loginToBook": "سجّل الدخول لحجز المواعيد",
  "welcomeBack": "مرحباً بعودتك",
  "joinBridge": "انضم إلى Bridge",
  "signInToContinue": "سجّل الدخول للمتابعة",
  "createAccount": "أنشئ حسابك",
  "fullName": "الاسم الكامل",
  "namePlaceholder": "محمد أحمد",
  "email": "البريد الإلكتروني",
  "password": "كلمة المرور",
  "minChars": "٦ أحرف على الأقل",
  "loading": "جارٍ التحميل...",
  "signIn": "تسجيل الدخول",
  "createAccountBtn": "إنشاء حساب",
  "noAccount": "ليس لديك حساب؟ سجّل الآن",
  "hasAccount": "لديك حساب بالفعل؟ سجّل الدخول"
},
"appointments": {
  "loading": "جارٍ تحميل المواعيد...",
  "title": "المواعيد",
  "bookNew": "موعد جديد",
  "bookAppointmentTitle": "حجز موعد",
  "service": "الخدمة",
  "booked": "تم حجز الموعد!",
  "confirmation": "ستتلقى تأكيداً قريباً.",
  "selectDate": "اختر التاريخ",
  "selectTime": "اختر الوقت",
  "notes": "ملاحظات (اختياري)",
  "notesPlaceholder": "أي طلبات أو معلومات محددة...",
  "booking": "جارٍ الحجز...",
  "confirm": "تأكيد الموعد",
  "yourAppointments": "مواعيدك",
  "none": "لا توجد مواعيد بعد",
  "noneSubtitle": "احجز موعدك الأول مع مزود الخدمة!",
  "notesLabel": "ملاحظات:",
  "passed": "انتهى هذا الموعد",
  "cancel": "إلغاء",
  "cancelConfirm": "هل أنت متأكد من إلغاء هذا الموعد؟"
},
"messages": {
  "loading": "جارٍ تحميل الرسائل...",
  "goBack": "العودة",
  "title": "الرسائل",
  "noConversations": "لا توجد محادثات بعد",
  "noConversationsSub": "ابدأ المراسلة مع مزودي الخدمات!",
  "serviceProvider": "مزود الخدمة",
  "typing": "يكتب...",
  "noMessages": "لا توجد رسائل بعد",
  "noMessagesSub": "ابدأ المحادثة!",
  "placeholder": "اكتب رسالة...",
  "selectConversation": "اختر محادثة",
  "selectConversationSub": "اختر محادثة من القائمة للبدء",
  "deleteConfirm": "حذف هذه الرسالة؟",
  "startConversation": "بدء محادثة"
},
  "community_places": {
  "title": "المفضلة المجتمعية",
  "subtitle": "اكتشف أماكن يوصي بها نيويوركيون — من الأكل الميسور إلى أماكن الدراسة الهادئة",
  "shareButton": "+ شارك مكاناً تحبه",
  "staffPick": "اختيار الفريق",
  "addedByCommunity": "أضافه أحد أفراد المجتمع",
  "getDirections": "الاتجاهات",
  "filterAll": "جميع الأماكن",
  "filterCoffee": "قهوة ودراسة",
  "filterFood": "أكل ميسور",
  "filterParks": "حدائق وترفيه",
  "filterWifi": "واي فاي مجاني",
  "filterCultural": "ثقافي وديني",
  "filterCommunity": "مراكز مجتمعية",
  "filterWork": "أماكن عمل مجانية",
  "noPlaces": "لم يتم العثور على أماكن. جرب فئة أخرى.",
  "selectPrompt": "اختر فئة لاستكشاف المفضلات المجتمعية",
  "selectSub": "مقاهي، حدائق، أكل ميسور والمزيد",
  "formTitle": "شارك مكاناً تحبه",
  "formName": "اسم المكان",
  "formNamePlaceholder": "مثال: مقهى البن",
  "formCategory": "الفئة",
  "formDescription": "الوصف",
  "formDescriptionPlaceholder": "لماذا تحب هذا المكان؟ (جملة أو جملتان)",
  "formAddress": "العنوان (اختياري)",
  "formAddressPlaceholder": "مثال: 123 الشارع الرئيسي، نيويورك",
  "formSubmit": "إرسال المكان",
  "formSubmitting": "جارٍ الإرسال...",
  "formSuccess": "شكراً! تم إرسال مكانك للمراجعة.",
  "formError": "فشل الإرسال. يرجى المحاولة مرة أخرى.",
  "pendingReview": "في انتظار المراجعة",
  "showing": "عرض",
  "of": "من",
  "places": "أماكن"
}
} as const;

export default translation;